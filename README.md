...
- **内网映射（EasyTier）**：no-tun 模式、不申请 VPN，组网设备经虚拟 IP 直达本机任意端口（如 5244），无需端口转发规则；支持对等节点、网络密钥、QUIC 代理；独立于 OpenList 服务启停（专属前台服务保活，OpenList 停止/崩溃不掉线）、设置页单独开关、开机随偏好自启；内置后台冻结/清理自愈
- **流量统计**：设置页查看应用进程收发流量（开机累计 / 服务会话 / 实时速率），基于 Android TrafficStats UID 级计数，不修改 OpenList 内核源码
